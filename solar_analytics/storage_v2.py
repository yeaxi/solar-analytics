...
class SolarAnalyticsV2Store:
    ...
    def get_runtime(self, key: str) -> Any | None:
        row = self.db.execute("SELECT value_json FROM v2_runtime_state WHERE key=?", (key,)).fetchone()
        return json.loads(row[0]) if row else None

    def ensure_lineage(self, *, contract_key: str, metadata: Mapping[str, Any], now: datetime) -> str:
        ...
        if current_id and current_key == contract_key:
            row = self.db.execute("SELECT lineage_id FROM v2_lineages WHERE lineage_id=?", (current_id,)).fetchone()
            if row:
        ...
        values = {
            ...
            "normalization_version": str(metadata.get("normalization_version", NORMALIZATION_VERSION)),
        ...
        if current_id and current_id != lineage_id:
            self.db.execute("UPDATE v2_lineages SET ended_at_utc=? WHERE lineage_id=? AND ended_at_utc IS NULL", (values["started_at_utc"], current_id))
    ...
    def ensure_snapshot_slot(
        ...
        *,
        ...
        target = target_local_date.isoformat() if isinstance(target_local_date, date) else str(target_local_date)
        ...
        values = (
            lineage_key, source_kind, snapshot_type, scheduled_at_utc.astimezone(UTC).isoformat(), target, timezone_name,
            ...
            observation_sequence, payload_sha256, adapter_version, normalization_version, metric_version,
            status, int(admissible), exclusion_reason, self._now(),
        )
    ...
    def insert_snapshot_periods(self, snapshot_slot_id: int, rows: list[Mapping[str, Any]]) -> None:
        self.db.executemany(
            ...
            [
                (
                    snapshot_slot_id,
                    row.get("interval_start_utc"), row["interval_end_utc"], row.get("energy_wh"),
                    row.get("duration_seconds"), int(bool(row.get("valid"))), row.get("exclusion_reason"),
                )
                for row in rows
            ],
        )
    ...
    def list_snapshot_slots(
        ...
        *,
        ...
        clauses: list[str] = []
        params: list[Any] = []
        for column, value in (("lineage_id", lineage_id), ("target_local_date", target_local_date), ("snapshot_type", snapshot_type), ("source_kind", source_kind)):
            if value is not None:
                clauses.append(f"{column}=?")
                params.append(value)
        query = "SELECT * FROM v2_snapshot_slots" + (" WHERE " + " AND ".join(clauses) if clauses else "") + " ORDER BY scheduled_at_utc"
    ...
    def snapshot_periods(self, snapshot_slot_id: int) -> list[dict[str, Any]]:
        return [dict(row) for row in self.db.execute("SELECT * FROM v2_snapshot_intervals WHERE snapshot_slot_id=? ORDER BY interval_end_utc", (snapshot_slot_id,)).fetchall()]

    def upsert_current_profile(self, *, source_kind: str, lineage_id: str | None, observed_at_utc: datetime, native_updated_at_utc: datetime | None, observation_sequence: int | None, payload_sha256: str | None, profile: Any, quality: Mapping[str, Any]) -> None:
        self.db.execute(
            ...
            (source_kind, lineage_id, observed_at_utc.astimezone(UTC).isoformat(), native_updated_at_utc.astimezone(UTC).isoformat() if native_updated_at_utc else None, observation_sequence, payload_sha256, self._json(profile), self._json(quality), self._now()),
        )

    def current_profile(self, source_kind: str = "native") -> dict[str, Any] | None:
        row = self.db.execute("SELECT * FROM v2_current_profile_cache WHERE source_kind=?", (source_kind,)).fetchone()
        if row is None:
            return None
        result = dict(row)
    ...
    def add_power_sample(self, timestamp: datetime, power_w: float | None, *, minutes: int = 30, max_gap_seconds: int = 900) -> None:
        current_ts = timestamp.astimezone(UTC) if timestamp.tzinfo else timestamp.replace(tzinfo=UTC)
            ...
            if previous_ts is not None and previous_power is not None and previous_ts < current_ts:
                ...
                if 0 < gap <= max_gap_seconds:
                    ...
                    while cursor < current_ts:
                        bucket_epoch = int(cursor.timestamp()) - (int(cursor.timestamp()) % bucket_seconds)
                        ...
                        if segment_seconds > 0:
                            ...
                            self.db.execute(
                                ...
                                (key, previous_power * segment_seconds / 3600.0, segment_seconds, 1, previous_power, "good", bucket_seconds * 0.8),
                            )
                        cursor = segment_end
        if power_w is not None:
            self.set_runtime("last_actual_sample", {"timestamp": current_ts.isoformat(), "power_w": float(power_w)})

    def integrate_accumulators(self, start_utc: datetime, end_utc: datetime) -> dict[str, float | int | str | None]:
        ...
        try:
            rows = self.db.execute("SELECT * FROM v2_accumulators WHERE interval_start_utc < ? ORDER BY interval_start_utc", (end.isoformat(),)).fetchall()
        except sqlite3.OperationalError:
            return {"energy_wh": None, "covered_seconds": 0.0, "sample_count": 0, "quality": "missing"}
        energy = 0.0
        covered = 0.0
        count = 0
        quality = "good"
        for row in rows:
            bucket_start = datetime.fromisoformat(row["interval_start_utc"].replace("Z", "+00:00")).astimezone(UTC)
            ...
            if row["quality"] != "good":
                quality = "gap"
        return {"energy_wh": energy if covered > 0 else None, "covered_seconds": covered, "sample_count": count, "quality": quality if covered else "missing"}
    ...
    def upsert_interval(self, payload: Mapping[str, Any]) -> None:
        ...
        self.db.execute(
            ...
            (data["lineage_id"], data["interval_start_utc"], data["interval_end_utc"], data["target_local_date"], data.get("forecast_energy_wh"), data.get("actual_energy_wh"), data.get("eligible_seconds", 0.0), data.get("actual_covered_seconds", 0.0), int(bool(data.get("forecast_valid"))), int(bool(data.get("actual_valid"))), int(bool(data.get("paired_valid"))), data.get("validity_reason", "unknown"), data.get("reconciliation_status"), self._json(data), self._now()),
        )

    def list_intervals(self, *, lineage_id: str | None = None, local_date: str | None = None, since: str | None = None) -> list[dict[str, Any]]:
        clauses=[]; params=[]
        for column,value in (("lineage_id",lineage_id),("target_local_date",local_date),("interval_end_utc",since)):
            if value is not None:
                clauses.append(f"{column}>=?" if column=="interval_end_utc" else f"{column}=?"); params.append(value)
        query="SELECT * FROM v2_intervals"+(" WHERE "+" AND ".join(clauses) if clauses else "")+" ORDER BY interval_start_utc"
        rows=[]
    ...
    def upsert_daily(self, payload: Mapping[str, Any]) -> None:
        ...
        self.db.execute(
            ...
            (data["lineage_id"], data["local_date"], data.get("morning_slot_id"), data.get("forecast_coverage",0.0), data.get("actual_coverage",0.0), data.get("paired_coverage",0.0), int(bool(data.get("valid_paired_day"))), data.get("reason","insufficient_data"), data.get("actual_kwh"), data.get("forecast_kwh"), data.get("signed_error_kwh"), data.get("absolute_error_kwh"), data.get("reconciliation_status"), self._json(data), self._now()),
        )

    def list_daily(self, *, lineage_id: str | None = None, since: str | None = None) -> list[dict[str, Any]]:
        clauses=[]; params=[]
        if lineage_id is not None: clauses.append("lineage_id=?"); params.append(lineage_id)
        if since is not None: clauses.append("local_date>=?"); params.append(since)
        query="SELECT * FROM v2_daily_comparisons"+(" WHERE "+" AND ".join(clauses) if clauses else "")+" ORDER BY local_date"
        return [dict(row) for row in self.db.execute(query,params).fetchall()]

    def save_accuracy(self, *, lineage_id: str, generated_at: datetime, window_days: int, valid_days: int, accuracy_ready: bool, payload: Mapping[str, Any]) -> None:
        self.db.execute(
    ...
    def latest_accuracy(self, lineage_id: str | None = None) -> dict[str, Any] | None:
        query="SELECT * FROM v2_accuracy_results"; params=[]
        if lineage_id is not None: query+=" WHERE lineage_id=?"; params.append(lineage_id)
    ...
    def list_backfill_runs(self) -> list[dict[str, Any]]:
        ...
        for row in self.db.execute("SELECT * FROM v2_backfill_runs ORDER BY created_at_utc").fetchall():
            item = dict(row)
    ...
    def insert_backfill_snapshot_periods(self, snapshot_id: int, rows: Sequence[Mapping[str, Any]]) -> None:
        self.db.executemany(
    ...
    def upsert_backfill_interval(self, *, run_id: str, lineage_id: str, local_date: str, payload: Mapping[str, Any]) -> None:
        data = dict(payload)
        self.db.execute(
    ...
    def upsert_backfill_daily(self, *, run_id: str, lineage_id: str, payload: Mapping[str, Any]) -> None:
        data = dict(payload)
        self.db.execute(
    ...
    def backup_to(self, destination: str | Path) -> None:
        destination_path=Path(destination); destination_path.parent.mkdir(parents=True,exist_ok=True)
        ...
        with self.transaction():
            ...
            try:
                cursor = self.db.execute(
                    ...
                    (datetime.combine(current.date() - timedelta(days=retention_days), time.min, tzinfo=UTC).isoformat(),),
                )
